...
function clickAgregarPentagrama() {
  ...
  props.cancion.pentagramas.push(
    nPentagrama
  )
}
...
<template>
  ...
  <div>
    Pentagramas <select>
      <option v-for="(pentagrama, index) in props.cancion.pentagramas" :key="index" :value="pentagrama">{{ pentagrama.instrumento }}</option>
    </select>
